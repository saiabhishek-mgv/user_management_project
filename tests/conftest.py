...
def admin_token(admin_user):
    ...
    token_data = {"sub": str(admin_user.email), "role": admin_user.role.name, "id": str(admin_user.id)}
    return create_access_token(data=token_data, expires_delta=timedelta(minutes=30))

@pytest.fixture(scope="function")
def manager_token(manager_user):
    token_data = {"sub": str(manager_user.email), "role": manager_user.role.name, "id": str(manager_user.id)}
    return create_access_token(data=token_data, expires_delta=timedelta(minutes=30))

@pytest.fixture(scope="function")
def user_token(user):
    token_data = {"sub": str(user.email), "role": user.role.name, "id": str(user.id)}
    return create_access_token(data=token_data, expires_delta=timedelta(minutes=30))
